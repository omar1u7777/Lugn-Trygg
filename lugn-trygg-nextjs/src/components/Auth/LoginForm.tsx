...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Expecting { access_token, user_id }
  ...
  const handleGoogleSignIn = () => {
    // Firebase Google sign-in requires firebase to be configured in this project.
    // For now show a helpful message; we can wire Firebase later.
    setError("Google sign-in is not configured in the migrated app yet.");
  };
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-semibold mb-4">Glömt lösenord</h3>
            <p className="text-sm text-slate-600">Funktionen för återställning av lösenord kommer att migreras separat.</p>
            <div className="mt-4 text-right">
              <button onClick={() => setShowForgotPassword(false)} className="btn btn-secondary">Stäng</button>
            </div>
          </div>
        </div>
      )}
