...
export const OfflineIndicator: React.FC<OfflineIndicatorProps> = ({
  ...
  if (variant === 'snackbar') {
    return (
      <Snackbar
        ...
        action={
          <Button color="inherit" size="small" onClick={() => setShowAlert(false)} aria-label="Dismiss offline status notification">Close</Button>
        }
      >
        <Alert
          ...
          aria-live="polite"
          aria-atomic="true"
          sx={{ borderRadius: 1, boxShadow: 2, minWidth: '300px' }}
        >
          {isSyncing ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <CircularProgress size={16} sx={{ color: 'inherit' }} aria-hidden="true" />
              <span>Syncing {unsyncedCount} unsaved item{unsyncedCount !== 1 ? 's' : ''}...</span>
            </Box>
          ) : (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, padding: '8px 12px', borderRadius: 1, backgroundColor: isOffline ? '#fff3cd' : '#d4edda', color: isOffline ? '#856404' : '#155724', fontSize: '12px', fontWeight: 500 }}>
      {isSyncing ? (
        <>
