...
try:
    from ..ml.mood_predictor import get_mood_predictor, MoodPrediction
    ML_AVAILABLE = True
except ImportError:
    ML_AVAILABLE = False
...
class AtRiskUser:
    ...
    last_mood_log: Optional[datetime]
...
class ProactiveInterventionService:
    ...
    async def _assess_user_risk(self, user_id: str) -> Optional[AtRiskUser]:
        """
    ...
    def _select_intervention_type(self, risk_factors: List[str], 
                                   predicted_mood: Optional[float]) -> InterventionType:
    ...
    async def _generate_intervention(self, user: AtRiskUser) -> Optional[Intervention]:
    ...
    def _generate_morning_message(self, user: AtRiskUser, context: dict) -> str:
        ...
        if 'declining_trend' in user.risk_factors:
            return (
                f"God morgon! Jag ser att du har haft det tufft. "
                f"Vill du göra en snabb check-in och se hur du mår idag? "
                f"Små steg framåt räknas."
            )
    ...
    def _generate_grounding_message(self, user: AtRiskUser, context: dict) -> str:
        ...
        if 'high_volatility' in user.risk_factors:
            return (
                f"Jag märker att du upplever stora svängningar i humöret. "
                f"Det kan vara utmattande. Vill du prova en 2-minuters grounding-övning "
                f"för att skapa lite stabilitet?"
            )
        
        return (
            f"Hej! Jag har en känsla av att du kanske behöver lite lugn just nu. "
            f"Vill du prova en snabb grounding-teknik?"
        )
    ...
    def _generate_mood_prompt(self, user: AtRiskUser, context: dict) -> str:
        ...
        if 'consecutive_negative_days' in user.risk_factors:
            return (
                f"Det har gått några dagar sedan du loggade ditt humör. "
                f"Jag tänker att det kan vara värdefullt att checka in - "
                f"oavsett hur du mår. Vill du ta 30 sekunder nu?"
            )
        
        return (
            f"Hej! Det var ett tag sedan du loggade ditt humör. "
            f"Vill du göra en snabb check-in? Det hjälper oss att följa dina mönster."
        )
    ...
    def _generate_values_message(self, user: AtRiskUser, context: dict) -> str:
        ...
        return (
            f"Hej! Ibland kan det hjälpa att påminna sig om vad som verkligen "
            f"är viktigt för oss. Vill du göra en kort övning om dina värderingar?"
        )
    
    def _generate_crisis_prevention_message(self, user: AtRiskUser, context: dict) -> str:
        """Generate crisis prevention message."""
        return (
            f"Jag ser att du har haft det svårt, och jag vill nå ut för att du "
            f"inte ska känna dig ensam. Det finns strategier som kan hjälpa. "
            f"Vill du öppna appen och titta på dina säkerhetsresurser?"
        )
    
    def _generate_celebration_message(self, user: AtRiskUser, context: dict) -> str:
        """Generate celebration message for positive trends."""
        return (
            f"Fantastiskt! Jag ser att du har haft en positiv trend i ditt humör. "
            f"Vad tror du har bidragit? Vill du spara detta i din framgångsjournal?"
        )
    ...
    async def _get_last_chat(self, user_id: str) -> Optional[datetime]:
...
# Singleton
_proactive_service: Optional[ProactiveInterventionService] = None
