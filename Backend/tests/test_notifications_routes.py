...
class TestSaveFcmToken:
    ...
    def test_missing_token_field(self, client, auth_csrf_headers):
        resp = client.post(f'{PREFIX}/fcm-token', json={}, headers=auth_csrf_headers)
        assert resp.status_code == 400
        data = resp.get_json()
        assert data['success'] is False

    def test_empty_string_token(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/fcm-token',
            json={'fcmToken': ''},
            headers=auth_csrf_headers,
    ...
    def test_null_token(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/fcm-token',
            json={'fcmToken': None},
            headers=auth_csrf_headers,
    ...
    def test_no_json_body(self, client, auth_csrf_headers):
        resp = client.post(f'{PREFIX}/fcm-token', headers=auth_csrf_headers)
        assert resp.status_code == 400

    def test_invalid_json(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/fcm-token',
            data='not-json',
    ...
    def test_long_token_accepted(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/fcm-token',
            json={'fcmToken': 'x' * 600},
            headers=auth_csrf_headers,
    ...
    def test_special_chars_token(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/fcm-token',
    ...
    def test_db_unavailable_returns_503(self, client, auth_csrf_headers):
    ...
    def test_firestore_write_exception(self, client, auth_csrf_headers):
        db = sys.modules['src.firebase_config'].db
...
class TestSendReminder:
    ...
    def test_user_not_found(self, client, auth_csrf_headers):
    ...
    def test_no_fcm_token_returns_sent_false(self, client, auth_csrf_headers):
    ...
    def test_empty_body_uses_defaults(self, client, auth_csrf_headers):
        _setup_user(fcm_token='tok')
        resp = client.post(f'{PREFIX}/send-reminder', json={}, headers=auth_csrf_headers)
        assert resp.status_code == 200

    def test_no_json_body(self, client, auth_csrf_headers):
        _setup_user(fcm_token='tok')
        resp = client.post(f'{PREFIX}/send-reminder', headers=auth_csrf_headers)
        assert resp.status_code == 200

    def test_db_unavailable(self, client, auth_csrf_headers):
        with patch('src.routes.notifications_routes.db', None):
            resp = client.post(
                f'{PREFIX}/send-reminder',
                json={'type': 'daily'},
    ...
    def test_fcm_firebase_error(self, client, auth_csrf_headers):
    ...
    def test_generic_exception(self, client, auth_csrf_headers):
        db = sys.modules['src.firebase_config'].db
    ...
    def test_unicode_message(self, client, auth_csrf_headers):
        _setup_user(fcm_token='tok')
        resp = client.post(
            f'{PREFIX}/send-reminder',
...
class TestScheduleDaily:
    ...
    def test_enable_with_time(self, client, auth_csrf_headers):
        resp = client.post(
    ...
    def test_disable(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/schedule-daily',
    ...
    def test_empty_body_defaults(self, client, auth_csrf_headers):
        """Empty JSON -> enabled=True, time='09:00'."""
        resp = client.post(f'{PREFIX}/schedule-daily', json={}, headers=auth_csrf_headers)
        assert resp.status_code == 200
        data = resp.get_json()
        assert data['data']['enabled'] is True
        assert data['data']['time'] == '09:00'

    def test_invalid_time_format(self, client, auth_csrf_headers):
        resp = client.post(
            f'{PREFIX}/schedule-daily',
    ...
    def test_exception(self, client, auth_csrf_headers):
        db = sys.modules['src.firebase_config'].db
...
class TestDisableAll:
    ...
    def test_success(self, client, auth_csrf_headers):
        resp = client.post(f'{PREFIX}/disable-all', headers=auth_csrf_headers)
        assert resp.status_code == 200
        data = resp.get_json()
        assert data['success'] is True
...
class TestNotificationSettings:
    ...
    def test_get_existing_user(self, client, auth_csrf_headers):
        _setup_user(
            fcm_token='tok',
            exists=True,
            extra={
        ...
        resp = client.get(f'{PREFIX}/settings', headers=auth_csrf_headers)
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_get_nonexistent_user_returns_defaults(self, client, auth_csrf_headers):
        _setup_user(exists=False)
        resp = client.get(f'{PREFIX}/settings', headers=auth_csrf_headers)
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_post_full_update(self, client, auth_csrf_headers):
    ...
    def test_post_partial_update_enabled_only(self, client, auth_csrf_headers):
        _setup_user(exists=True)
        resp = client.post(
            f'{PREFIX}/settings',
    ...
    def test_post_invalid_time(self, client, auth_csrf_headers):
        _setup_user(exists=True)
        resp = client.post(
            f'{PREFIX}/settings',
    ...
    def test_post_empty_body(self, client, auth_csrf_headers):
        _setup_user(exists=True)
        resp = client.post(f'{PREFIX}/settings', json={}, headers=auth_csrf_headers)
        assert resp.status_code == 200
        data = resp.get_json()
        assert data['data']['updated'] is True

    def test_db_unavailable_get(self, client, auth_csrf_headers):
        with patch('src.routes.notifications_routes.db', None):
            resp = client.get(f'{PREFIX}/settings', headers=auth_csrf_headers)
        assert resp.status_code == 503

    def test_db_unavailable_post(self, client, auth_csrf_headers):
        with patch('src.routes.notifications_routes.db', None):
            resp = client.post(
                f'{PREFIX}/settings',
...
class TestIntegration:
    ...
    def test_save_token_then_send_reminder(self, client, auth_csrf_headers):
        resp1 = client.post(
            f'{PREFIX}/fcm-token',
    ...
    def test_schedule_then_disable(self, client, auth_csrf_headers):
        resp1 = client.post(
            f'{PREFIX}/schedule-daily',
            json={'enabled': True, 'time': '09:00'},
            headers=auth_csrf_headers,
        ...
        resp2 = client.post(f'{PREFIX}/disable-all', headers=auth_csrf_headers)
        assert resp2.status_code == 200

    def test_multiple_token_saves(self, client, auth_csrf_headers):
        for i in range(5):
            resp = client.post(
                f'{PREFIX}/fcm-token',
