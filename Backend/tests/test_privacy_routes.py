...
def test_get_privacy_settings_returns_defaults(client, auth_csrf_headers, mock_auth_service, mock_db):
    user_doc = mock_db.collection('users').document(TEST_USER_ID)
    user_doc.get.return_value = MagicMock(exists=True, to_dict=lambda: {})

    response = client.get(f'/api/privacy/settings/{TEST_USER_ID}', headers=auth_csrf_headers)
...
def test_update_privacy_settings_filters_allowed_keys(client, auth_csrf_headers, mock_auth_service, mock_db):
    user_doc = mock_db.collection('users').document(TEST_USER_ID)
    ...
    update_payload = user_doc.update.call_args.args[0]
    assert 'privacy_settings.unknown' not in update_payload
    assert update_payload['privacy_settings.shareAnonymizedData'] is True


def test_export_user_data_streams_json_file(client, auth_csrf_headers, mock_auth_service, mock_db, mocker):
    user_doc = mock_db.collection('users').document(TEST_USER_ID)
    ...
    response = client.post(f'/api/privacy/export/{TEST_USER_ID}', headers=auth_csrf_headers)
...
def test_delete_user_data_requires_confirmation(client, auth_csrf_headers, mock_auth_service):
    response = client.delete(f'/api/privacy/delete/{TEST_USER_ID}', headers=auth_csrf_headers)

    assert response.status_code == 400
    assert 'Confirmation required' in response.get_json()['message']


def test_delete_user_data_executes_full_cleanup(client, auth_csrf_headers, mock_auth_service, mock_db, mocker):
